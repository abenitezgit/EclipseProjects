    ...
    public void run() {
        ...
        */
        ...
        for (int i=-findHour; i<=findHour; i++) {
            ...
            vSQL = "select "+iteratorHour+" horaAgenda,ageID, month, dayOfMonth, dayOfWeek, weekOfYear, weekOfMonth, hourOfDay from tb_agenda where "
                    + "     ageEnable=1 "
        ...
        for (int i=-findMinutes; i<=0; i++) {
             ...
             */
            vSQL = "select "+iteratorMinute+" horaAgenda,ageID, month, dayOfMonth, dayOfWeek, weekOfYear, weekOfMonth, hourOfDay from tb_agenda where "
                    + "     ageEnable=1 "
                    + "     and substr(month,"+posmonth+",1) = '1'"
                    + "     and substr(dayOfMonth,"+posdayOfMonth+",1) = '1'"
                    + "     and substr(dayOfWeek,"+posdayOfWeek+",1) = '1'"
        	...
        	switch (gDatos.getServerInfo().getDbType()) {
	        	case "ORA":
	                vSQL =  "select gr.GRPID, gr.GRPDESC, to_char(gr.UFECHAEXEC,'rrrr-mm-dd hh24:mi:ss') UFECHAEXEC, gr.USTATUS, gr.STATUS, gr.LASTNUMSECEXEC,  gr.CLIID, ha.HORID, \n" +
	                		"pr.PROCID, pr.NORDER, pr.CRITICAL \n" +
	                        "from \n" +
	                        "  process.tb_grupos gr, \n" +
	                        "  process.tb_HORAAGENDA ha, \n" +
	                        "  process.tb_cliente cl, \n" +
	                        "  process.TB_HORARIO ho, \n" +
	                        "  process.TB_AGENDA ag, \n" +
	                        "  process.TB_PROCGRUPO pr \n" +
	                        "where \n" +
	                        "  gr.GRPID = pr.GRPID \n" +
	                        ...
	                        "  and gr.ENABLE =1 \n" +
	                        "  and gr.LASTNUMSECEXEC < '"+gDatos.getLstActiveAgendas().get(i).getNumSecExec()+"' \n" +
	                        "  and ha.AGEID='"+gDatos.getLstActiveAgendas().get(i).getAgeID()+"'\n" +
	                        "  and cl.ENABLE = 1 \n" +
	                        "  and ag.AGEENABLE = 1 \n" +
	                        "  and ho.HORENABLE = 1 \n" +
	                        "  and ha.HORENABLE = 1 \n" +
